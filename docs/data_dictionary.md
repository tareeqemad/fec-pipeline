...
| `contributor_employer` | string | The donor's employer, **unified per company** (legal suffixes stripped, synonyms/typos merged). Joins to `employers.csv`.`employer_name`. **Non-individuals carry a placeholder naming their entity class instead**: committees → `CAMPAIGN/COMMITTEE`, organizations → `ORGANIZATION` (these are markers, never companies, and never appear in `employers.csv`). |
