"""The incremental merge dedups sub_ids left behind by a run that crashed
between the output write and the tracker update."""
import pandas as pd
...
def test_no_overlap_appends_all():
    ...
    out = clean._merge_incremental(existing, new)
    assert out['sub_id'].tolist() == ['1', '2', '3']
    assert out['v'].tolist() == ['a', 'b', 'c']


def test_crash_window_duplicates_keep_freshest():
    # Prior run wrote 2,3 to the output but crashed before the tracker update,
    # so this run re-cleaned them — the re-cleaned (last) version must win.
...
def test_identical_rerun_is_idempotent():
    existing = pd.DataFrame({'sub_id': ['1', '2'], 'v': ['a', 'b']})
    out = clean._merge_incremental(existing, existing.copy())
    assert len(out) == 2
    assert not out['sub_id'].duplicated().any()
