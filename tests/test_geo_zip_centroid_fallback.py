...
def boca(monkeypatch):
    monkeypatch.setattr(geo, "_zip_centroids", lambda: {"33496": Z33496, "33434": Z33434})
...
def test_failed_street_lookup_falls_back_to_the_zip_centroid(boca, monkeypatch):
    ...
    monkeypatch.setattr(geo, "city_level", lambda *_args: (*BOCA_CITY, "US"))
...
def test_po_box_keeps_the_town_pin_inside_its_zip(boca, monkeypatch):
    monkeypatch.setattr(geo, "_zip_neighbour_km", lambda _zip: 10.0)   # a rural-sized ZIP
    monkeypatch.setattr(geo, "city_level", lambda *_args: (*BOCA_CITY, "US"))
...
def test_po_box_town_pins_on_the_real_zip_reference(tmp_path, key, town_pin, replaced):
    ...
    expected = geo._zip_centroids()[key.split("|")[3]] if replaced else town_pin
...
def test_city_zip_conflict_gives_no_coordinate_for_a_street(boca, monkeypatch):
    monkeypatch.setattr(geo, "census", lambda *_args: (None, None, None))
    monkeypatch.setattr(geo, "nominatim", lambda *_args: (None, None, None))
    monkeypatch.setattr(geo, "city_level", lambda *_args: (30.33, -81.65, "US"))
...
def test_zip_centroid_in_another_state_is_not_used(monkeypatch):
    ...
    monkeypatch.setattr(geo, "_zip_centroids", lambda: {"33496": Z33496})
