...
def test_web_search_stores_successes_and_skips_failed_calls(monkeypatch):
    stored = []
    cache = _Cache()

    def search(_client, _model, _system_prompt, prompt):
        if prompt == "BROKEN":
            raise RuntimeError("temporary failure")
        return json.dumps([{"name": prompt}]), 0.25
    ...
    assert found == 2
    assert {item for item, _ in stored} == {"FIRST", "SECOND"}
    assert all(result["name"] == item for item, result in stored)
