...
def test_cli_uses_committees_csv_by_default(monkeypatch):
    calls = []
    ...
    assert pull_cli.main(["--period", "2025"]) == 0
    assert [call["committee_id"] for call in calls] == list(tracked)
    assert {call["period"] for call in calls} == {2026}


def test_cli_missing_api_key_stops_before_committees(monkeypatch):
    calls = []
    monkeypatch.setattr(
