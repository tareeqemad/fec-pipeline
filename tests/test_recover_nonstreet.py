"""Tests for _recover_nonstreet_from_donor — fill fragment streets from donor history."""
import numpy as np
import pandas as pd
...
def test_state_single_fallback_when_city_mismatches():
    # Fragment row's city is garbled ("GOLDEN BCH") so name+city+state misses,
    # but the donor has exactly ONE clean street in FL -> safe to recover.
    df = pd.DataFrame([
