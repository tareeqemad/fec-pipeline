...
from fec.config.data import INDIV_NAME_RE, MISSING_VALUES, ORG_KEYWORDS
...
def _restore_individual_occupations(
    ...
    needs_fix = restored.index[
        df.loc[restored.index, "contributor_occupation"].isin(OCCUPATION_FIXES)
    ]
    if len(needs_fix) > 0:
