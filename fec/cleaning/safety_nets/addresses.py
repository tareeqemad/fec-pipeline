"""cleaning/safety_nets/addresses.py — address (city/state/ZIP) safety-net fixes."""
from __future__ import annotations

import numpy as np
import pandas as pd
from fec.config.constants import (
    FOREIGN_CITIES_NO_US_STATE, AMBIGUOUS_CITIES,
)


def _fill_null_city_from_zip(df: pd.DataFrame) -> int:
    """J. contributor_city NULL → fill from ZIP lookup."""
    _ZIP_CITY = {'10022': 'NEW YORK'}
    null_city = df['contributor_city'].isna()
    n = 0
    if null_city.any():
        for idx in df[null_city].index:
            z = str(df.at[idx, 'contributor_zip'] or '')
            if z in _ZIP_CITY:
                df.at[idx, 'contributor_city'] = _ZIP_CITY[z]
                n += 1
    return n


def _fix_foreign_addresses(df: pd.DataFrame) -> int:
    """AF. Foreign city names with fake US states → NaN city.
    e.g. REHOVOT/CA, NETANYA/WA — clearly non-US addresses.
    For ambiguous cities (LONDON, PARIS, etc.) only flag when
    the state doesn't match a known US location with that name."""
    city = df['contributor_city'].fillna('').str.strip().str.upper()
    state = df['contributor_state'].fillna('').str.strip().str.upper()

    # Unambiguous foreign cities — always flag
    unambiguous = FOREIGN_CITIES_NO_US_STATE - set(AMBIGUOUS_CITIES.keys())
    mask_foreign = city.isin(unambiguous)

    # Ambiguous cities — only flag when state doesn't match known US locations
    for city_name, valid_states in AMBIGUOUS_CITIES.items():
        is_this_city = city == city_name
        wrong_state = ~state.isin(valid_states)
        mask_foreign = mask_foreign | (is_this_city & wrong_state)

    n = int(mask_foreign.sum())
    if n:
        df.loc[mask_foreign, 'contributor_city'] = np.nan
    return n


def _fix_garbage_city_names(df: pd.DataFrame) -> int:
    """AG. Garbage city names (commas, ethnicity markers) → NaN.
    e.g. 'WHITE, NOT OF HISPANIC ORIGIN', 'JERUSALEM, ISRAEL'."""
    city = df['contributor_city'].fillna('')
    # Cities containing commas (should never happen — city is a single name)
    has_comma = city.str.contains(',', na=False)
    # Census race/ethnicity artifact
    has_ethnicity = city.str.contains(r'NOT OF HISPANIC|HISPANIC ORIGIN', case=False, na=False)
    mask = has_comma | has_ethnicity
    n = int(mask.sum())
    if n:
        df.loc[mask, 'contributor_city'] = np.nan
    return n


def _fix_pr_zip_wrong_state(df: pd.DataFrame) -> int:
    """AH. Puerto Rico/VI ZIP with wrong state → fix state.
    ZIP 006xx-009xx = PR, ZIP 008xx (00801-00851) = VI."""
    zip5 = df['contributor_zip'].fillna('')
    state = df['contributor_state'].fillna('')

    # VI ZIPs: 00801-00851
    is_vi_zip = zip5.str.match(r'^008[0-4]\d$|^00850$|^00851$', na=False)
    vi_wrong = is_vi_zip & (state != 'VI')

    # PR ZIPs: 00600-00799, 00900-00999 (excluding VI range)
    is_pr_zip = zip5.str.match(r'^00[679]\d{2}$', na=False) & ~is_vi_zip
    pr_wrong = is_pr_zip & (state != 'PR')

    n = 0
    if vi_wrong.any():
        df.loc[vi_wrong, 'contributor_state'] = 'VI'
        df.loc[vi_wrong, 'state_name'] = 'VIRGIN ISLANDS'
        n += int(vi_wrong.sum())
    if pr_wrong.any():
        df.loc[pr_wrong, 'contributor_state'] = 'PR'
        df.loc[pr_wrong, 'state_name'] = 'PUERTO RICO'
        n += int(pr_wrong.sum())
    return n
