...
def _zero(name, sql, label, severity=CRIT):
...
def _equal(name, sql_a, sql_b, label, severity=CRIT):
...
def _positive(name, sql, label, severity=CRIT):
...
VIEW_CHECKS += [
    ...
    _zero("v_contributions_cleaned: amount matches fact",
    ...
    _zero("v_contributions_cleaned: year = year(date)",
          "SELECT count(*) FROM v_contributions_cleaned v JOIN contributions c ON c.sub_id=v.sub_id "
          "WHERE v.contributor_year <> EXTRACT(YEAR FROM c.receipt_date)::int", "row(s) with wrong year"),
    _zero("v_contributions_cleaned: individual name = 'LAST, FIRST'",
    ...
    _zero("v_contributions_cleaned: literal 'NULL' surname preserved",
    ...
    _zero("v_contributions_cleaned: recipient_committee resolves",
    ...
    _zero("v_contributions_cleaned: entity_type matches donor",
...
VIEW_CHECKS += [
    _equal("v_donor_profile: one row per donor",
           "SELECT count(*) FROM v_donor_profile", "SELECT count(*) FROM donors", "rows"),
    _zero("v_donor_profile: total matches stats",
    ...
    _zero("v_donor_profile: count matches stats",
          "SELECT count(*) FROM v_donor_profile p JOIN v_donor_stats s ON s.donor_id=p.donor_id "
          "WHERE p.donation_count IS DISTINCT FROM s.donation_count", "donor(s) with wrong count"),
    _zero("v_donor_profile: non-contributors have no stats",
    ...
    _zero("v_donor_profile: identity matches donors",
    ...
    _zero("v_donor_profile: current_employer matches employment view",
...
for view, owner_join in [
    ...
    VIEW_CHECKS.append(_zero(f"{view}: one row per donor",
                             f"SELECT count(*) FROM (SELECT donor_id FROM {view} GROUP BY donor_id HAVING count(*)>1) t",
                             "donor(s) with >1 row"))
    VIEW_CHECKS.append(_zero(f"{view}: row belongs to the donor",
                             f"SELECT count(*) FROM {view} v WHERE {owner_join}", "row(s) not owned by donor"))
VIEW_CHECKS += [
    _zero("v_donor_current_address: is the newest contribution's address",
    ...
    _zero("v_donor_newest_address: one row per donor",
          "SELECT count(*) FROM (SELECT donor_id FROM v_donor_newest_address GROUP BY donor_id HAVING count(*)>1) t",
          "donor(s) with >1 row"),
    _zero("v_donor_newest_address: row belongs to the donor",
...
VIEW_CHECKS += [
    ...
    _positive("leader_committees: membership is populated",
              "SELECT count(*) FROM leader_committees", "junction row(s)"),
    _zero("leader_committees: every row maps to a visible leader",
...
VIEW_CHECKS += [
    _equal("mv_donor_profile: same row count as view",
           "SELECT count(*) FROM mv_donor_profile", "SELECT count(*) FROM v_donor_profile", "rows"),
    _zero("mv_donor_profile: in sync with view (per donor)",
...
def _build() -> list[Check]:
    ...
    checks += [
        ...
        _zero("dedup: donor_employments (donor,employer,occupation)",
        ...
        Check("sub_id fits BIGINT", CRIT, lambda cur: (
    ...
    checks += [
        Check("money conserved (contributions = stats = flat)", CRIT, _conserved),
        _zero("v_donor_stats = direct aggregate",
        ...
        _zero("v_donor_stats: election_cycles correct",
              "SELECT count(*) FROM v_donor_stats vs JOIN "
        ...
        _zero("v_donor_stats: first <= last donation",
    ...
    checks += [
        ...
        _zero("active status implies an employer",
    ...
    checks += [
        ...
        _zero("election cycles even & in range",
