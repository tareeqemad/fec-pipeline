"""Structured query-correctness checks for the loaded fec_db.

ONE source of truth for "does every query return the right data", consumed two ways:

  * tests/test_db_live.py — pytest parametrizes over CHECKS, asserting each (CI/automation)
  * fec/database/healthcheck.py — run_checks iterates CHECKS against the live
    fec_db (crit → FAIL, warn → WARN) and adds a few live-only extras
    (required views exist, geocode coverage %, matview freshness, CSV total)

Each Check is (name, category, severity, fn). `fn(cur) -> (ok: bool, detail: str)`.
`crit` checks are correctness (a failure means a query returns wrong data) and are
asserted by pytest; `warn` checks are data-quality signals shown in the panel but
not failed by CI (legitimate edge cases exist — territories, refunds, …).
...
class Check:
    ...
    def run(self, cur) -> tuple[bool, str]:
        try:
            ...
            return False, f"error: {type(e).__name__}: {str(e).strip()[:200]}"
...
def _zero(name, category, sql, label, severity=CRIT):
    ...
    def fn(cur, sql=sql, label=label):
        n = _scalar(cur, sql)
        return n == 0, (f"0 {label}" if n == 0 else f"{n:,} {label}")
...
def _equal(name, category, sql_a, sql_b, label, severity=CRIT):
    ...
    def fn(cur, a=sql_a, b=sql_b, label=label):
        va, vb = _scalar(cur, a), _scalar(cur, b)
        return va == vb, (f"{label}: {va:,} = {vb:,}" if va == vb else f"{label}: {va:,} ≠ {vb:,}")
    return Check(name, category, severity, fn)
...
def _build() -> list[Check]:
    ...
    # ── 1. integrity ──────────────────────────────────────────────────
    for t in ("donors", "contributions", "committees", "addresses", "employers"):
        C.append(_positive(f"rows: {t}", "integrity", f"SELECT count(*) FROM {t}", "row(s)"))
    for child, fk, parent, pk in _ORPHANS:
        C.append(_zero(
            f"orphans: {child}.{fk}", "integrity",
            f"SELECT count(*) FROM {child} c LEFT JOIN {parent} p ON p.{pk}=c.{fk} "
    ...
    # ── 3. v_contributions_cleaned (flat view) ────────────────────────
    C += [
    ...
    for view, owner_join in [
        ...
        C.append(_zero(f"{view}: one row per donor", "sub_views",
                       f"SELECT count(*) FROM (SELECT donor_id FROM {view} GROUP BY donor_id HAVING count(*)>1) t",
                       "donor(s) with >1 row"))
        C.append(_zero(f"{view}: row belongs to the donor", "sub_views",
    ...
    C += [
        ...
        # committee membership lives in the leader_committees junction (the
        # per-committee page filters it directly) — not on v_leaders. Check the
        # junction is sound: populated, and every row maps to a visible leader.
    ...
    C += [
        ...
        # Moved from healthcheck.py when it became a consumer of this registry —
        # data-quality signals, not correctness (hence WARN):
    ...
    # ── 9. value sanity (data itself well-formed) ─────────────────────
    C += [
        _zero("no future receipt dates", "sanity",
              "SELECT count(*) FROM contributions WHERE receipt_date > CURRENT_DATE", "future-dated row(s)"),
        # WARN, not CRIT: schema.sql enforces only NOT NULL on election_cycle —
        # there is no CHECK on parity or range — so an odd / out-of-range cycle
        # is dirty upstream data (a quality signal), not a query returning
        # wrong results. (Severity drifted CRIT-vs-WARN between the old
        # healthcheck copy and this registry; resolved here, once.)
