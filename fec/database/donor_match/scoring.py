...
def _is_typo(a: str, b: str) -> bool:
    """Check if two strings differ by at most 1 edit (Levenshtein distance <= 1)."""
...
def _are_cross_group_candidates(norm1: str, norm2: str) -> bool:
    """
    Check if two norm_names (LAST|FIRST) should be compared cross-group.
    Requirements: same last name + first names are similar.

    Similarity criteria (ANY of):
      - Known nickname pair (NICKNAME_MAP canonical match)
      - Levenshtein <= 1 (single-char keyboard error: JEFREY->JEFFREY)
      - Levenshtein = 2 AND one name is a prefix of the other
        (truncation: JOSH->JOSHUA, PHIL->PHILIP, MARV->MARVIN)

    BLOCKED: name pairs that look similar but are different people
    (cross-gender, unrelated names that happen to be Levenshtein <= 1).
    """
...
def _is_surname_variant(last1: str, last2: str) -> bool:
    """Two DIFFERENT surnames that look like spelling variants of one person.

    Catches typos (SHTREMBERG/SHTEREMBERG), space/hyphen/dot forms
    (DE TOLEDO/DETOLEDO, GAL-OR/GALOR), and an initial glued onto the surname
    (A SPIEGEL/SPIEGEL). Used ONLY by the surname-variant matching phase, which
    additionally requires a strong geographic signal (same street or ZIP5)
    before it will merge — so this can stay permissive without fusing families.
    """
    if not last1 or not last2 or last1 == last2:
        return False
    squash = lambda s: s.replace(" ", "").replace("-", "").replace(".", "")
    if squash(last1) == squash(last2):          # DE TOLEDO == DETOLEDO
        return True
...
def compute_score(p1: dict, p2: dict, name_freq: int) -> tuple:
    """
    Compute merge confidence score between two record profiles.
    Returns (score, signals_list).

    RULES:
      - At least one GEOGRAPHIC signal (street, city, ZIP, or state) is
        required for any merge.  Employer match alone is not enough
        because large companies have employees in many states.
      - Name rarity alone is never enough.
    """
    score = 0
    signals = []
    has_geo = False
    ...
    # Occupation field (category) — corroboration when the same name moves or
    # keeps two homes. Within a state it just adds points; for a VERY rare name
    # (freq<=2) a consistent occupation also unlocks a cross-state merge in the
    # safety net below (a retiree's summer/winter address). occ_conflict (two
    # different real careers) signals two different people and blocks that path.
    ...
    occ_compatible = False                              # consistent field / life-status
    ...
    if (not has_employer and has_geo and name_freq <= 3
        ...
        bonus = 25
        score += bonus
        signals.append(f"retired_no_emp_rare_geo(+{bonus})")

    # SAFETY: require geographic corroboration
    has_corroboration = has_geo or (has_employer and has_geo)
    ...
    if not has_corroboration:
        if has_employer and not has_geo:
            ...
            # Very rare name (appears in exactly two places) + a consistent
            # occupation/life-status (both retired, same field, or a retirement
            # transition) + no career conflict \u2192 trust it as one person who moved
            # or keeps two homes (a retiree's summer/winter address), even across
            # state lines. This is the only path that merges without geographic
            # corroboration; gated on freq<=2 so two different people who share a
            # common name are never fused.
            distinctive = (
